...
TestUtil.assert_any(len(features), len(config.inference_config.weights))
for i in range(len(features)):
    features[i] = (
        features[i]
        * config.inference_config.weights[i]
        / np.sum(config.inference_config.weights)
    )
features = np.concatenate(features, axis=1)
features: Any = normalize(features)
batch_idxs = InferenceFactory.get_batch_idxs(len(data.test), n_batches=20)
y_pred_df = InferenceFactory.get_concat_inference_y_pred_df(
    batch_idxs, features, config.inference_config.threshold, posting_ids
)
